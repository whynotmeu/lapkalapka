...
2. Изменение hostname от имени lapka
...
PID(1496)TTY(?)STAT(SS)TIME(0:00)MAJFL(0)TRS(0)DRS(15436)RSS(8768)MEM(0.1)
...
- Опция **"f"** выдает такие колонки:
  ...
  - PID - идентификатор процесса (1496);
...
- общая загрузка системы: Load average 1мин: 0.5 5мин. 0.5 15мин. 0.00
- общее количество процессов: 99 total, 1 running, 98 sleeping, 0 stopped, 0 zombie

- загрузка cpu: 
   - us — пользовательские процессы (высокое значение данного показателя может указывать, в том числе, на проблемы в коде сайта, необходимость его оптимизации): 0.2.
   - sy — процессы ядра: 0.0.
   - id — неиспользуемые ресурсы: 99.6.
   - wa — операции ввода/вывода, т.е. дисковые операции: 0.0.

- загрузка памяти:
   - Mem: 7797.0 total 6169.7 free, 207.4 used, 1419.9 buff/cache
   - Swap: 4096.0 total, 4096.0 free, 0.0 used, 7330.0 avail Mem
...
>Рисунок №42. Вывод df.
 ...
 1.  "df" Для корневого раздела (/):
   ...
   - размер занятого пространства: 7826232.
   - размер свободного пространства: 29536984.
...
- размер раздела: 38G
- размер занятого пространства: 7.5G
- размер свободного пространства: 29G
- процент использования: 21%
...
- Время последней успешной авторизации: Jan 9 03.35.07
- Имя пользователя: lapka
